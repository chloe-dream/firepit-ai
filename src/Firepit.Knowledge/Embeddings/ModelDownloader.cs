...
// — one download per machine, shared by every project's knowledge index.
// Ported from the-fishbowl (Fishbowl.Search.ModelDownloader).
//
...
public sealed class ModelDownloader
{
    ...
    // True once both files are on disk. EmbeddingService uses this to decide
    // whether to initialise the pipeline or throw
    ...
    private sealed record ModelFile(string UpstreamPath, string LocalName, string? Sha256);
}
